...
import { LayoutState, PoolInfo, RootState, TokenInfo, TokenState } from "app/store/types";
...
const PoolDetail: React.FC<PoolDetailProps> = (props: PoolDetailProps) => {
  const { children, className, poolInfo, ...rest } = props;
  ...
  const token0FormatOpts: MoneyFormatterOptions = {
    symbol: poolInfo?.token0.symbol,
    compression: poolInfo?.token0.decimals,
  };
  const token1FormatOpts: MoneyFormatterOptions = {
    symbol: poolInfo?.token1.symbol,
    compression: poolInfo?.token1.decimals,
  };
  ...
  const getPoolSizeValue = () => {
    if (!poolInfo) return <span className={classes.textWrapper}>-</span>;
    const { token1Reserve, token0Reserve } = poolInfo.pool;
    return (
      <span className={classes.textWrapper}>
        <span className={classes.textColoured}>
          {moneyFormat(token0Reserve, token0FormatOpts)}
        </span>
        <span className={classes.textColoured}>
          {moneyFormat(token1Reserve, token1FormatOpts)}
        </span>
      </span>
    )
  };
  const getShareValue = () => {
    if (!poolInfo) return <span className={classes.textWrapper}>-</span>;
    const { token0Reserve, token1Reserve } = poolInfo.pool;

    const contributionPercentage = BIG_ZERO;
    const share = contributionPercentage.shiftedBy(-2);
    const tokenContribution = share.times(token1Reserve);
    const zilContribution = share.times(token0Reserve);
    return (
      <span className={classes.textWrapper}>
        {/* (<span className={classes.textColoured}>{moneyFormat(zilContribution, zilFormatOpts)}</span>
        ZIL +
        <span className={classes.textColoured}>{moneyFormat(tokenContribution, formatOpts)}</span>) */}
      </span>
    )
  };
  const getUserPoolShare = () => {
    if (!poolInfo) return "";
    // const { contributionPercentage } = poolInfo.pool;
    const contributionPercentage = BIG_ZERO;
  ...
  const getPoolTokenValue = () => {
    ...
    return (
      <span className={classes.textWrapper}>
        {/* <span className={classes.textColoured}>{moneyFormat(new BigNumber(rate).times(poolToken?.pool?.exchangeRate || 0), { ...zilFormatOpts, compression: 0 })}</span>
        {" "}ZIL + {" "}
        <span className={classes.textColoured}>{moneyFormat(new BigNumber(rate).shiftedBy(poolToken?.decimals || 0), formatOpts)}</span>
        {" "}{token!.symbol} */}
      </span>
    )
  };
  const getExchangeRateValue = () => {
    return <></>
    // if (!pool) return <span className={classes.textWrapper}>-</span>;
    // const zilToken = tokenState.tokens[ZIL_ADDRESS];
    // const rate = pool.exchangeRate.shiftedBy(token!.decimals - zilToken.decimals);
